...
/* Rank tests on transcripts, as per the ethanol study:

Try base model (no covariates)
And adaptation of Model 37 */

data xs_rank;
set cc.xscript_wghtd_rank_by_cell_subj;
...
model xscript_rank = transcript_id status transcript_id*status sex status*sex plate p1|p2 / ss3;
lsmeans transcript_id*status / pdiff cl;
output out=exon_rank_resid_m37_w predicted=pred residual=resid student=stu;
ods output modelanova=xs_rank_glm_m37_w diff=pdiff_m37_w ;
run;
...
output out=exon_rank_resid_m37_w2 predicted=pred residual=resid student=stu;
ods output modelanova=xs_rank_glm_m37_w2 slicedanova=slice_m37_w ;
run;
...
where Flag_for_removal_ambig3=0;
class transcript_id status sex plate;
model xscript_rank2 = transcript_id status transcript_id*status sex status*sex plate p1|p2 / ss3;
lsmeans transcript_id*status / pdiff cl;
output out=exon_rank_resid_m37_wA predicted=pred residual=resid student=stu;
ods output modelanova=xs_rank_glm_m37_wA diff=pdiff_m37_wA ;
run;
...
model xscript_rank2 = transcript_id status transcript_id*status sex status*sex plate p1|p2 / ss3;
lsmeans transcript_id*status / slice=transcript_id;
output out=exon_rank_resid_m37_w2A predicted=pred residual=resid student=stu;
ods output modelanova=xs_rank_glm_m37_w2A slicedanova=slice_m37_wA ;
run;


/* Make permenant */

data cc.xs_wght_ranktest_m37_anova;
set xs_rank_glm_m37_w;
run;

data cc.xs_wght_ranktest_m37_resid;
set exon_rank_resid_m37_w;
run;

data cc.xs_wght_ranktest_m37_pdiffs;
set pdiff_m37_w;
run;


data cc.xs_wght_ranktest_m37_anova2;
set xs_rank_glm_m37_w2;
run;

data cc.xs_wght_ranktest_m37_resid2;
set exon_rank_resid_m37_w2;
run;


data cc.xs_wght_ranktest_m37_slices;
set slice_m37_w;
run;
...
data cc.xs_wght_ranktest_m37_anova3;
set xs_rank_glm_m37_wA;
run;

data cc.xs_wght_ranktest_m37_resid3;
set exon_rank_resid_m37_wA;
run;

data cc.xs_wght_ranktest_m37_pdiffs3;
set pdiff_m37_wA;
run;


data cc.xs_wght_ranktest_m37_anova3a;
set xs_rank_glm_m37_w2a;
run;

data cc.xs_wght_ranktest_m37_resid3a;
set exon_rank_resid_m37_w2a;
run;


data cc.xs_wght_ranktest_m37_slices3;
set slice_m37_wA;
run;
...
data ranktest;
  set cc.xs_wght_ranktest_m37_anova2;
...
data cc.m37_anova_fdr_ranktest_wght;
