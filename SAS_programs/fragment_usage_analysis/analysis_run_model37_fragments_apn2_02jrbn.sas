...
data xs2keep_&cell.;
  set oldcc.flag_fragment_apn_gt0_ge2_&cell.;
  where flag_&cell._CTL_apn_ge2=1 and flag_&cell._T1D_apn_ge2=1;
  keep event_id;
run;

data counts;
  set cclocal.frag_junc_q3_norm_counts;
...
data frag2gene;
  set oldcc.events_summary_by_fragment_apn2;
...
data cc.m37_anova_results_frag_ds;
  set anova;
run;

data cc.m37_anova_residuals_frag_ds;
  set resid_by_cell;
run;

data cc.m37_anova_resid_failrate_frag_ds;
  set resid_fail_count;
run;

data cc.m37_model_fit_stats_frag_ds;
 set fit_stats;
run;

data cc.m37_model_lsmeans_frag_ds;
 set lsmeans_test;
run;

data cc.m37_model_lsmeans_diff_frag_ds;
 set diffs_test;
run;

data cc.m37_model_constrasts_frag_ds;
 set  model_contrasts;
run;

data cc.m37_model_estimates_frag_ds;
 set model_estimates;
run;
...
data ds_model_gene_de;
  set cc.m37_anova_results_frag_ds;
  where effect ="status" and probf ne .;
...
data cc.m37_anova_fdr_gene_frag_de;
...
data ds_model;
  set cc.m37_anova_results_frag_ds;
  where effect ="event_id*status" and probf ne .;
  keep gene_id cell_type fvalue probf ;
run;
...
data cc.m37_anova_fdr_gene_frag_ds;
   set flag_ds_fdr;
run;
