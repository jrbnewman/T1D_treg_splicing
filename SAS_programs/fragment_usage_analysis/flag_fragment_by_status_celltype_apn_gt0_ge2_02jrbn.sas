...
data frag_counts_tall;
  set cclocal.fragment_counts_by_sample;
run;
...
%macro flagapn(cell);
   ...
   proc sort data=frag_counts_tall;
      by sample_id;
   ...
   data counts_w_key ;
     merge design_roz (in=in1) frag_counts_tall (in=in2);
     by sample_id;
   ...
   %macro flagapn2(apn,apnvar);
      ...
      proc means data=flag_apn noprint;
         ...
         %else %do;
            ...
            %end;
      output out=flag_frag_by_trt mean=trt_perc;

      /* If proportion is 50% or greater then flag transcript as on */

     data flag_trt_perc;
        set flag_frag_by_trt;
        if trt_perc >= 0.5 then flag_frag_on=1;
        else if trt_perc = 0 then flag_frag_on=0;
        else flag_frag_on=.;
        keep event_id status flag_frag_on;
     run;
     ...
     data flag_frag_by_trt_apn&apnvar.;
   ...
   proc sort data=flag_frag_by_trt_apn0;
      by event_id;
   proc sort data=flag_frag_by_trt_apn2;
      by event_id;
   proc sort data=flag_frag_by_trt_apn5;
      by event_id;
   run;

   data flag_frag_by_trt_&cell.;
     merge flag_frag_by_trt_apn0 (in=in1) flag_frag_by_trt_apn2 (in=in2) flag_frag_by_trt_apn5 (in=in3);
     by event_id;
     if in1 and in2;
   run;
...
data cc.flag_fragment_apn_gt0_ge2_127;
   set flag_frag_by_trt_127;
run;

data cc.flag_fragment_apn_gt0_ge2_400;
   set flag_frag_by_trt_400;
run;

data cc.flag_fragment_apn_gt0_ge2_425;
   set flag_frag_by_trt_425;
run;

data cc.flag_fragment_apn_gt0_ge2_426;
   set flag_frag_by_trt_426;
run;
